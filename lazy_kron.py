...
class ZeroKron(torch.optim.Optimizer):
    """Kron optimizer with optional lazy preconditioner updates.

    This optimizer maintains the same mathematical operations as your original Kron optimizer,
    while allowing for lazy updates of the preconditioners.
    ...
    Args:
        ...
        preconditioner_update_interval (int, optional): Number of steps between preconditioner updates.
            If set to 1, updates are performed every step (default: 1).
    ...
    def __init__(
        ...
        if not isinstance(preconditioner_update_interval, int) or preconditioner_update_interval < 1:
            raise ValueError(f"Invalid preconditioner_update_interval: {preconditioner_update_interval}")
        ...
        defaults = dict(
            ...
            preconditioner_update_interval=preconditioner_update_interval,
        ...
        for (
            ...
            if len(state) == 0:
                ...
                state["momentum_buffer"] = torch.zeros_like(
                    param, dtype=group["mu_dtype"] or param.dtype
                )
    ...
    def step(self, closure=None):
        # Implement the optimizer step with optional lazy updates
        loss = None
        ...
        for group in self.param_groups:
            preconditioner_update_interval = group.get("preconditioner_update_interval", 1)
        ...
        for (
            ...
            # Lazy update of preconditioners
            if do_update and state["step"] % group["preconditioner_update_interval"] == 0:
                _update_precond(
                    state["Q"],
                    state["exprs"],
        ...
        for group in self.param_groups:
            for param in group["params"]:
                if param.grad is not None:
                    ...
                    param.grad.data /= self.world_size  # Average gradients
    ...
    def _sync_params(self):
        ...
        for group in self.param_groups:
            for param in group["params"]:
                ...
                dist.broadcast(param.data, src=0)  # Broadcasting from rank 0
